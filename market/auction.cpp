...
int auction::update_statistics(){
	...
	for(current = stats; current != 0; current = current->next){
		...
		if(current->stat_type == SY_MEAN){
			...
			double gameStd = gameTheory.getStd();
			printf("stdev=%f gameStd=%f\n", current->value, gameStd);
		}
...
void auction::clear_market(void)
{
	...
	cleared_frame.start_time;
	cleared_frame.end_time;
	char timestart[128];
	gl_printtime(cleared_frame.start_time, timestart, 127);
	char timeend[128];
	gl_printtime(cleared_frame.start_time, timeend, 127);
	string start(timestart);
	string end(timeend);
	//printf("call getpredictions with timestamp : %s , \n", start);
	//gameTheory.setPredictions(timestart);
	//gameTheory.setOnePrediction(timestart);
	gameTheory.setSupplyPrediction(timestart);
}

...
int controller::init(OBJECT *parent){
	...
	fetch_double(&pOutTemperature, "outdoor_temperature", parent);
...
//	next_run = gl_globalclock + (TIMESTAMP)(period - fmod(gl_globalclock+period,period));
	...
	if (control_mode == CN_DOUBLE_RAMP){
		...
		double* indoorTemp = gl_get_double_by_name(parent2, "air_temperature");
		double* outdoorTemp = gl_get_double_by_name(parent2, "outdoor_temperature");
		double* heatingSet = gl_get_double_by_name(parent2, "heating_setpoint");
		double* coolingSet = gl_get_double_by_name(parent2, "cooling_setpoint");
		double* designHeatingCap = gl_get_double_by_name(parent2, "design_heating_capacity");
		double* designCoolingCap = gl_get_double_by_name(parent2, "design_cooling_capacity");
		double* initHeatingCOP = gl_get_double_by_name(parent2, "heating_COP");
		double* initCoolingCOP = gl_get_double_by_name(parent2, "cooling_COP");

		market->gameTheory.addElement(id, *heatingSet, *coolingSet, *indoorTemp, *outdoorTemp, *designCoolingCap, *designHeatingCap, *initCoolingCOP, *initHeatingCOP);
	}
...
TIMESTAMP controller::presync(TIMESTAMP t0, TIMESTAMP t1){
	...
	//////////////////////////////////////////////////////////////// game /////////////////////////////////////////////////////////////
	market->gameTheory.calculateTotalLoad();
	...
	if(control_mode == CN_RAMP){
		...
		*/
		...
		if(lastmkt_id != *pMarketId){
			...
			double lamda = market->gameTheory.getLamda();
			double stdev = market->gameTheory.getStd();
			if(fabs(*pStd) < bid_offset){
				*pCoolingSetpoint = cooling_setpoint0;
				*pHeatingSetpoint = heating_setpoint0;
			} else {
...
TIMESTAMP controller::postsync(TIMESTAMP t0, TIMESTAMP t1){
	...
	////////////////////////////////////////////////////////////////// game ////////////////////////////////////////////////////////////////////////
	if (control_mode == CN_DOUBLE_RAMP){
		//printf("controller postsync heating_demand=%f cooling_demand=%f pMonitor=%f, pCoolingDemand=%f\n", *pHeatingDemand, *pCoolingDemand, *pMonitor, *pOutTemperature);
		market->gameTheory.updateNodeTemperatures(id, *pMonitor, *pOutTemperature);
		market->gameTheory.updateNodeLoads(id, *pHeatingDemand, *pCoolingDemand);
	}
	return rv;
}
